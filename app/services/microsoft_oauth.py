...
class _WrongTenant(Exception):
    """Raised internally when Azure says the grant was for a different tenant."""
...
async def exchange_code_for_tokens(code: str, redirect_uri: Optional[str] = None) -> dict:
    ...
    async def _try_exchange(tenant: str) -> dict:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            logger.warning(
                "Microsoft OAuth token exchange failed for tenant '%s': %s",
                tenant, body,
            )
    ...
    try:
        ...
        logger.info("Microsoft OAuth: falling back to tenant '%s'", fallback)
