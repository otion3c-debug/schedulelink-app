"""ScheduleLink API - Production-Ready Scheduling SaaS.

Key Features:
- User registration/login with JWT auth
- Public booking pages (NO auth required)
- Double-booking prevention with EXCLUSIVE transaction locking
- Google Calendar integration (OAuth2)
- Stripe subscription ($5/month with 7-day trial)
- Email confirmations to both client and host
...
from .database import init_db, get_db_dependency, seed_working_hours, USE_POSTGRES
from .models import (
...
# CORS - allow all origins for flexibility with ngrok URLs
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def get_config():
    ...
    return {
        ...
        "stripe_configured": stripe_api.is_stripe_configured(),
        "google_configured": calendar_api.is_configured(),
        "base_url": BASE_URL
    }
...
def register(user: UserCreate, db: Any = Depends(get_db_dependency)):
...
def login(credentials: UserLogin, db: Any = Depends(get_db_dependency)):
...
def get_me(current_user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "subscription_status": current_user.get("subscription_status") or "free",
...
def google_auth_redirect(current_user: dict = Depends(get_current_user)):
    ...
    if not calendar_api.is_configured():
...
def google_auth_callback(
    ...
    try:
        ...
        if state:
            ...
            print(f"[Google OAuth] Connected for user {state}")
...
def disconnect_google(
    ...
    print(f"[Google OAuth] Disconnected for user {current_user['id']}")
...
def update_working_hours(
    ...
    print(f"[Working Hours] Updated for user {current_user['id']}")
...
def update_settings(
    ...
    if updates:
        ...
        print(f"[Settings] Updated for user {current_user['id']}")
...
def get_user_access_token(user: dict, db: Any) -> Optional[str]:
...
def parse_busy_datetime(dt_str: str) -> datetime:
    """Parse datetime string from various formats to datetime object."""
    # Remove Z suffix and convert to naive datetime
...
def get_public_profile(username: str, db: Any = Depends(get_db_dependency)):
    ...
    while current_date <= end:
        ...
        if wh and wh["is_enabled"]:
            ...
            while slot_start + timedelta(minutes=meeting_duration) <= end_time:
                ...
                # Check if slot is in the past (with 30 min buffer)
                ...
                for busy_start_str, busy_end_str in busy_times:
                    try:
                        busy_start = parse_busy_datetime(busy_start_str)
                        busy_end = parse_busy_datetime(busy_end_str)
...
def create_booking(
    ...
    This acquires a RESERVED lock before checking for conflicts, ensuring
    ...
    # CRITICAL: Use transaction locking to prevent double-booking race conditions
    try:
        if USE_POSTGRES:
            # PostgreSQL: Use FOR UPDATE to lock rows
            db.execute("BEGIN")
            ...
            # SQLite: Use BEGIN IMMEDIATE for proper locking
        ...
        print(f"[Booking] Created booking {booking_id} for user {user['id']}")
...
def cancel_booking_by_client(
    ...
    if booking.get("google_event_id"):
        ...
        access_token = get_user_access_token(user_for_token, db)
        if access_token:
    ...
    print(f"[Booking] Cancelled by client: {booking['id']}")
...
def cancel_booking_by_host(
    ...
    print(f"[Booking] Cancelled by host: {booking_id}")
...
def create_checkout(
    ...
    checkout_url = stripe_api.create_checkout_session(
        customer_id,
        f"{FRONTEND_URL}/#/settings?billing=success&tier=pro",
...
async def stripe_webhook(request: Request, db: Any = Depends(get_db_dependency)):
        ...
        if customer_id and subscription_id:
            ...
            try:
                ...
                if stripe_module.api_key:
                    ...
                    price_id = (sub.get("items", {}) or {}).get("data", [{}])[0].get("price", {}).get("id")
                    # Check for Pro+ price via environment variable (extend later)
                    pro_plus_price_id = os.getenv("STRIPE_PRICE_ID_PRO_PLUS", "")
                    if pro_plus_price_id and price_id == pro_plus_price_id:
                        subscription_status = "pro_plus"
                    else:
                        subscription_status = "pro"
            except Exception as e:
                print(f"[Stripe Webhook] Could not determine tier: {e}")
                subscription_status = "pro"
            
            db.execute(
                """UPDATE users SET is_paid = 1, subscription_status = ?, stripe_subscription_id = ?
                   WHERE stripe_customer_id = ?""",
                (subscription_status, subscription_id, customer_id)
            )
            print(f"[Stripe Webhook] User upgraded to {subscription_status} (customer: {customer_id})")
...
async def serve_spa(full_path: str):
    """Serve static files or the SPA index.html.
    
    This catch-all route is defined AFTER all /api routes, so API routes
    take precedence. For any non-API path:
    - If it's a real file (JS, CSS, images), serve it
    - Otherwise, serve index.html for SPA client-side routing
    
    CRITICAL: /api/* paths that reach here were not matched by any API route,
    so they should 404, NOT serve index.html.
    """
    # Reject unhandled /api/* paths - they should NOT serve index.html
    if full_path.startswith("api/") or full_path == "api":
        raise HTTPException(status_code=404, detail="API endpoint not found")
    
    # Check if the requested path is an actual file
    if full_path:  # Non-root path
        file_path = os.path.join(frontend_path, full_path)
        if os.path.isfile(file_path):
            return FileResponse(file_path)
    
    # For root or any non-file path, serve index.html (SPA routing)
